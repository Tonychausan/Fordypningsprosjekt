\documentclass[12pt]{scrreprt}
\usepackage[utf8]{inputenc}


\usepackage{natbib}
\usepackage{graphicx}
\usepackage{tikz}
\usepackage{pgfplots}
\pgfplotsset{compat=1.12}
\usepackage{textcomp}


\usepackage{amsmath} % Math
\usepackage{hyperref} % Hyperlinks
\usepackage[noabbrev]{cleveref} % Clickalbe hyperlinks
\usepackage[parfill]{parskip} % Remove intent on paragraph
\usepackage{caption} % add captions
\usepackage{url} % URLs

\usepackage{glossaries} % for abbreviation list

\usepackage{mathptmx} % times new roman font
\usepackage{setspace} % line space

\usepackage{tocbibind} % fix for wrong hyperlink for toc, tof and tot

% Remove underfull hbox warnings for bib
\usepackage{etoolbox}
\apptocmd{\thebibliography}{\raggedright}{}{}



\usepackage{xcolor}

% Make pages dark and white text
\pagecolor[rgb]{0.3, 0.3, 0.3}
\color[rgb]{0.9,0.9,0.9}

% Edit the meta.tex file to change title and author names
\input{meta.tex}


\makeglossaries
\input{appendix/abbreviations.tex}


\begin{document}
% The title page
\input{title.tex}

\input{content/00-Abstract.tex}
\thispagestyle{empty}

\clearpage
\pagenumbering{Roman}
\setcounter{page}{1}


\tableofcontents
\clearpage

\listoffigures
\clearpage

\listoftables
\clearpage

\pagenumbering{arabic}
\setcounter{page}{1}

% Main matter - edit corresponding file under content/ to change
\input{content/01-Introduction.tex}
\input{content/02-Background.tex}
\input{content/03-MyoArmband.tex}
\input{content/04-Theory.tex}
\input{content/05-Methodology.tex}
\input{content/XV-Results.tex}
\input{content/XX-Discussion.tex}
\input{content/XY-Conclusion.tex}




% Bibliography - edit references.bib and use the \cite command in text
\renewcommand{\bibname}{References}
\bibliographystyle{plain}
\bibliography{references}

\clearpage

\addcontentsline{toc}{chapter}{Appendix}
\input{appendix/appendix.tex}
\pagenumbering{arabic}% resets `page` counter to 1
\renewcommand*{\thepage}{A-\arabic{page}}


\glsaddall % make it possible to print glossary without references
\phantomsection
\addcontentsline{toc}{section}{Abbreviations}
\printglossary[title=Abbreviations, nonumberlist, nopostdot=true] % print the glossary




\end{document}


